 ...
 * @version 2014/10/31
 * - bug fix: extraData was being nullptred accidentally in Vertex::resetData()
 * @version 2014/10/23
...
std::ostream& operator<<(std::ostream& out, const Vertex& v)
{
    ...
    out << ", parent=" << (v.parent == nullptr ? std::string("nullptr") : v.parent->name);
...
{
    out << "Edge{start=";
    if (edge.start == nullptr) {
        out << "nullptr";
    } else {
        out << edge.start->name;
    }
    out << ", finish=";
    if (edge.finish == nullptr) {
        out << "nullptr";
    } else {
        out << edge.finish->name;
    }
...
double BasicGraph::getWeightOfEdge(Vertex* v1, Vertex* v2) const
{
    Edge* e = BasicGraph::getEdge(v1, v2);
